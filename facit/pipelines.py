...
class FacitPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('facit.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `facit`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM facit WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO facit (date, title, link, content)"
